    ...
    module Identity
      class Builder
        class KeystoneV3
          ...
          def initialize(ems)
            @service = ems.connect(:service => "Identity", :orange_endpoint_type => 'adminURL')
          ...
          def find_or_create_projects
            @data.projects.each do |project|
              orange_project = (find(@service.projects.all(:domain_id => domain_id), project.slice(:name)) ||
                                   create_project(@service.projects, @projects, domain_id, project))
              @projects << orange_project
            end
          end
